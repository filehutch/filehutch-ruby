...
module Assetboar
  ...
  class DirectUploadsController < Assetboar.config.direct_upload_parent_controller.constantize
    before_action :authorize_assetboar_upload!

    def create
      upload = Assetboar.client.create_upload(
      ...
      render_assetboar_error(e)
    end

    def complete
      file = Assetboar.client.complete_upload(params[:id])
    ...
    def authorize_assetboar_upload!
      authorizer = Assetboar.config.authorize_direct_upload
      if authorizer
        args = [ self, params[:policy].to_s ].first(authorizer.arity.negative? ? 2 : authorizer.arity)
        return if instance_exec(*args, &authorizer)
      end

      message = authorizer ? "Not allowed to upload here" : "Direct uploads are disabled: set Assetboar.config.authorize_direct_upload"
      render json: { error: { code: "forbidden", message: message } }, status: :forbidden
    end

    def render_assetboar_error(error)
