...
module Assetboar
  ...
  class Configuration
    ...
    def initialize
      @api_key = ENV["ASSETBOAR_API_KEY"]
      @url = ENV.fetch("ASSETBOAR_URL", DEFAULT_URL)
      @open_timeout = 5
      @read_timeout = 30
      @write_timeout = 120
      @logger = nil
      @user_agent = "assetboar-ruby/#{VERSION} ruby/#{RUBY_VERSION}"
      @authorize_direct_upload = nil
      @direct_upload_parent_controller = "ApplicationController"
    end

    def validate!
      raise ConfigurationError, "Assetboar.config.api_key is missing (set ASSETBOAR_API_KEY)" if api_key.nil? || api_key.to_s.strip.empty?
      raise ConfigurationError, "Assetboar.config.url is missing (set ASSETBOAR_URL)" if url.nil? || url.to_s.strip.empty?
      self
    end
  end
end
